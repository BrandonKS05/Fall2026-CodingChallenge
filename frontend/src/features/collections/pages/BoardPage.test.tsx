...
const items = [itemFixture({ caption: 'Oak island' }), itemFixture({ id: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', caption: 'Black taps', position: 1 })];
const detailPath = `/api/collections/${board.id}`;

function renderBoard(routes: Parameters<typeof stubApi>[0]) {
  const api = stubApi({ 'GET /api/auth/me': { body: { user: { id: 'u1', email: 'ada@example.com', displayName: 'Ada', createdAt: board.createdAt } } }, 'GET /api/collections': { body: { collections: [board] } }, ...routes });
...
describe('BoardPage', () => {
  ...
  it('removes an image optimistically and the refetch agrees', async () => {
    ...
    const api = renderBoard({
      [`GET ${detailPath}`]: () => ({ body: { collection: { ...board, itemCount: remaining.length }, items: remaining } }),
    ...
    await waitFor(() =>
      expect(api.calls.some((call) => call.method === 'DELETE' && call.path.endsWith(items[0]!.id))).toBe(true),
    );
  ...
  it('explains private boards to visitors and hides controls on read-only boards', async () => {
    renderBoard({ [`GET ${detailPath}`]: { status: 403, body: { error: { code: 'FORBIDDEN', message: 'This board is private' } } } });
    expect(await screen.findByRole('heading', { name: 'This board is private' })).toBeInTheDocument();
    vi.unstubAllGlobals();

    renderBoard({ [`GET ${detailPath}`]: { body: { collection: { ...board, role: 'viewer' }, items } } });
