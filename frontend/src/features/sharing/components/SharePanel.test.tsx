...
const owner = { userId: 'u1', email: 'ada@example.com', displayName: 'Ada', role: 'owner', joinedAt: board.createdAt };
const friend = { userId: 'u2', email: 'grace@example.com', displayName: 'Grace', role: 'editor', joinedAt: board.createdAt };
...
describe('SharePanel', () => {
  ...
  it('lets the owner create a link and shows it as a copyable URL', async () => {
    ...
    expect(await screen.findByRole('textbox', { name: 'Share link' })).toHaveValue(`${window.location.origin}/s/abc123XYZ_-`);
    expect(api.calls.some((call) => call.method === 'POST' && call.path.endsWith('/share-link'))).toBe(true);
  });
  ...
  it('invites by email, changes roles, and removes members', async () => {
    ...
    const api = renderPanel({
      ...
      [`POST /api/collections/${board.id}/members`]: (call) => {
        ...
        members = [...members, { ...friend, userId: 'u3', email: body.email, displayName: 'Linus', role: body.role }];
        return { status: 201, body: members[members.length - 1] };
      },
      [`PATCH /api/collections/${board.id}/members/u2`]: (call) => {
        members = members.map((m) => (m.userId === 'u2' ? { ...m, role: (call.body as { role: string }).role } : m));
    ...
    await userEvent.type(screen.getByRole('textbox', { name: 'Invite by email' }), 'linus@example.com');
    await userEvent.click(within(screen.getByRole('radiogroup', { name: 'Role for the invitation' })).getByRole('radio', { name: 'Viewer' }));
    await userEvent.click(screen.getByRole('button', { name: 'Invite' }));
    await waitFor(() => expect(within(list).getAllByRole('listitem')).toHaveLength(3));
    expect(api.calls.find((call) => call.method === 'POST')?.body).toEqual({ email: 'linus@example.com', role: 'viewer' });

    await userEvent.click(within(screen.getByRole('radiogroup', { name: 'Role for Grace' })).getByRole('radio', { name: 'Viewer' }));
  ...
  it('shows the unknown-email message from the API', async () => {
    renderPanel({ [`POST /api/collections/${board.id}/members`]: { status: 404, body: { error: { code: 'NOT_FOUND', message: 'No account with that email' } } } });
    await userEvent.click(screen.getByRole('button', { name: 'Share' }));
    await userEvent.type(await screen.findByRole('textbox', { name: 'Invite by email' }), 'ghost@example.com');
    await userEvent.click(screen.getByRole('button', { name: 'Invite' }));
  ...
  it('gives non-owners the link, the list, and a way to leave', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Share' }));
    expect(await screen.findByRole('textbox', { name: 'Share link' })).toHaveValue(`${window.location.origin}/s/zzz`);
