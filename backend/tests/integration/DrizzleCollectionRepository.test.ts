...
import { DrizzleCollectionRepository } from '../../src/infrastructure/db/repositories/DrizzleCollectionRepository.js';
import { DrizzleMembershipRepository } from '../../src/infrastructure/db/repositories/DrizzleMembershipRepository.js';
...
describe.skipIf(!RUN_DB_TESTS)('DrizzleCollectionRepository + DrizzleMembershipRepository (postgres)', () => {
  ...
  beforeEach(async () => {
    ...
    ownerId = (await users.create({ email: 'owner@x.com', displayName: 'Owner', passwordHash: 'h' })).id;
    otherId = (await users.create({ email: 'other@x.com', displayName: 'Other', passwordHash: 'h' })).id;
  });
  ...
  it('builds summaries with owner name, counts, and the viewer role', async () => {
    ...
    expect(asOwner).toMatchObject({ ownerDisplayName: 'Owner', itemCount: 0, previewImageIds: [], role: 'owner' });
    ...
    expect(await collections.findSummary('00000000-0000-0000-0000-000000000000', null)).toBeNull();
  });
  ...
  it('updates, sets unique share slugs, and reports missing boards', async () => {
    ...
    await expect(collections.setShareSlug(second.id, 'abc123')).rejects.toBeInstanceOf(ConflictError);
    await collections.setShareSlug(board.id, null);
    expect(await collections.findByShareSlug('abc123')).toBeNull();

    const missing = '00000000-0000-0000-0000-000000000000';
    await expect(collections.update(missing, { title: 'x' })).rejects.toBeInstanceOf(NotFoundError);
  });
  ...
  it('manages memberships with owner-first ordering and conflict detection', async () => {
    ...
    expect(members.map((m) => [m.displayName, m.role])).toEqual([['Owner', 'owner'], ['Other', 'editor']]);
    ...
    await expect(memberships.updateRole(board.id, otherId, 'viewer')).rejects.toBeInstanceOf(NotFoundError);
  });
});
