...
import { asLogger, createPinoLogger } from '../../infrastructure/logging/pinoLogger.js';
...
function buildApp(exposeInternals: boolean) {
  ...
  app.get('/upstream', () => {
    throw new UpstreamError('Image provider is unreachable', 503);
  });
  app.get('/boom', () => {
...
describe('error handler domain mapping', () => {
  it.each([
    ...
    ['/upstream', 502, 'UPSTREAM_ERROR', 'Image provider is unreachable'],
