...
describe('ShareService', () => {
  ...
  it('revoking returns an unlisted board to private but leaves public boards public', async () => {
    ...
    expect(await repos.collections.findById(boardId)).toMatchObject({
      shareSlug: null,
      visibility: 'private',
    });
  ...
  it('invites accounts by email and announces it', async () => {
    ...
    await expect(share.invite(boardId, owner, 'friend@x.com', 'viewer')).rejects.toBeInstanceOf(
      ConflictError,
    );
    await expect(share.invite(boardId, owner, 'ghost@x.com', 'viewer')).rejects.toBeInstanceOf(
      NotFoundError,
    );
    await expect(share.invite(boardId, friend, 'stranger@x.com', 'viewer')).rejects.toBeInstanceOf(
      ForbiddenError,
    );
  });
  ...
  it('shows the member list to members only', async () => {
    ...
    expect((await share.listMembers(boardId, friend)).map((m) => m.role)).toEqual([
      'owner',
      'viewer',
    ]);
  ...
  it('changes roles for everyone but the owner', async () => {
    ...
    await expect(share.updateRole(boardId, owner, owner, 'viewer')).rejects.toBeInstanceOf(
      InvalidOperationError,
    );
    await expect(share.updateRole(boardId, friend, friend, 'editor')).rejects.toBeInstanceOf(
      ForbiddenError,
    );
  });
  ...
  it('lets owners remove members and members leave, but never removes the owner', async () => {
    await share.invite(boardId, owner, 'friend@x.com', 'editor');
    await expect(share.removeMember(boardId, stranger, friend)).rejects.toBeInstanceOf(
      ForbiddenError,
    );
    await expect(share.removeMember(boardId, owner, owner)).rejects.toBeInstanceOf(
      InvalidOperationError,
    );
