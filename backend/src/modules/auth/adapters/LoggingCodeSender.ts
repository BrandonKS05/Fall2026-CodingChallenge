...
export class LoggingCodeSender implements EmailSender, SmsSender {
  constructor(
    private readonly logger: Logger,
    private readonly channel: 'email' | 'sms',
  ) {}

  async send(message: { to: string; subject?: string; text?: string; body?: string }) {
    this.logger.warn(
      { to: message.to, channel: this.channel, message: message.text ?? message.body },
