...
| ----------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /api/auth/register`           | `{ email, handle, password, displayName }` | 201 `{ user }` and sets the cookie. Passwords need 10+ characters with a letter and a number; handles are 3-24 lowercase letters, numbers, and underscores. 409 if the email or the handle is taken.                                                                                              |
...
| `PATCH /api/auth/me`                | required                                   | `{ displayName?, handle?, bio?, preferences? }` (bio up to 160 characters). A handle may move once and then settles for two weeks (400 before that); a taken one is 409. A preference patch merges into what is stored, so flipping one switch leaves the others alone. Returns the updated user. |
| `GET /api/auth/handle-available`    | none                                       | `?handle=` — answers `{ handle, available }`. Open, because the sign-up form asks before an account exists, and rate limited like the other credential routes.                                                                                                                                    |
...
| `POST /api/collections/:id/like`   | required | Like a board you can see; owners cannot like their own (400). Returns the board with `likeCount` and `likedByViewer`, and the owner is notified.                                                                                                                                                                            |
| `DELETE /api/collections/:id/like` | required | Remove your like. Returns the board.                                                                                                                                                                                                                                                                                        |
